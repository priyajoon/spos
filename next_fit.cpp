...
void next(vector<block> &mem,int processsize,int processid,int &last_alloc_block){
	int n=mem.size();
	for(int i=last_alloc_block;i<n;i++){
    ...
    for(int i=0;i<last_alloc_block;i++){
        if(!mem[i].allocated && mem[i].size>=processsize){
            mem[i].allocated=true;
            last_alloc_block=i;
            mem[i].size_alloc=processsize;
    ...
    while(true){
    	...
    	switch(ch){
    		case 1:
    			...
    			for(int i=0;i<a;i++){
    				cin>>processsize;
    				next(mem,processsize,i+1,last_alloc_block);
    			}
